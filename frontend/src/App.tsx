...
//   /* ---------------- 1. INITIAL STATE (Refresh = New ID) ---------------- */
//   const [sessions, setSessions] = useState<Session[]>(() => {
//     const stored = localStorage.getItem("sessions");
//     return stored ? JSON.parse(stored) : [];
//   });

//   // This ensures that every refresh starts a brand new thread
...
//   /* ---------------- 4. ACTIONS (New, Delete, Rename) ---------------- */
//   const startNewChat = () => {
//     const id = `sess_${Date.now()}`;
//     setActiveSession(id);
//     setMessages([]);
//     // Only add to sidebar if not already present
...
//       if (!exists) {
//         return [{ id: activeSession, title: userText.slice(0, 25) }, ...prev];
//       }
//       return prev.map(s => 
//         s.id === activeSession && s.title === "New Chat" 
//         ? { ...s, title: userText.slice(0, 25) } 
//         : s
//       );
//     });
...
//       {/* Sidebar */}
//       <aside className="w-72 bg-[#171717] text-gray-300 p-4 flex flex-col shadow-2xl border-r border-white/5">
//         <button 
//           onClick={startNewChat} 
//           className="flex items-center justify-center gap-2 p-3 border border-gray-700 rounded-xl mb-6 hover:bg-gray-800 transition-all font-medium text-sm"
//         >
//           <Plus size={16} /> New Chat
//         </button>

//         <div className="flex-1 overflow-y-auto space-y-1 custom-scrollbar">
//           <p className="text-[10px] text-gray-500 font-bold uppercase px-2 mb-2 tracking-[0.15em]">History</p>
//           {sessions.map(s => (
//             <div 
//               key={s.id} 
...
//                 <input 
//                   autoFocus 
//                   className="bg-transparent outline-none w-full text-white border-b border-blue-500" 
...
//                     <button onClick={(e) => {e.stopPropagation(); setSessions(prev => prev.map(x => x.id === s.id ? {...x, isEditing: true} : x))}} className="hover:text-white"><Pencil size={14} /></button>
...
//       {/* Main Chat Area */}
//       <main className="flex-1 flex flex-col items-center bg-[#fdfdfd] relative">
//         <div ref={scrollRef} className="flex-1 w-full max-w-3xl p-8 overflow-y-auto space-y-6">
//           {messages.length === 0 && (
//             <div className="h-full flex flex-col items-center justify-center opacity-20 mt-32">
//               <h1 className="text-4xl font-bold">Internal GPT</h1>
//               <p className="mt-2">Ask anything about company documents</p>
//             </div>
//           )}
//           {messages.map((m, i) => (
//             <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
//               <div className={`p-4 rounded-2xl max-w-[85%] shadow-sm leading-relaxed ${
...
//             <textarea 
//               className="w-full p-4 pr-14 bg-white border border-gray-200 rounded-2xl resize-none outline-none focus:border-gray-400 transition-all" 
//               rows={1} 
//               value={q} 
//               onChange={e => setQ(e.target.value)} 
//               onKeyDown={e => e.key === "Enter" && !e.shiftKey && (e.preventDefault(), send())} 
//               placeholder="Ask your company documents..." 
//             />
//             <button 
//               onClick={send} 
//               disabled={loading} 
//               className={`absolute right-3 bottom-3 h-9 w-9 rounded-xl flex items-center justify-center transition-all ${
//                 loading ? "bg-gray-200 text-gray-400" : "bg-black text-white hover:scale-105 active:scale-95"
//               }`}
//             >
//               <Send size={18} />
//             </button>
...
//             Corporate Knowledge Base Access Only
//           </p>
//         </div>
//       </main>
//     </div>
